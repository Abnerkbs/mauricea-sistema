...
const App = () => {
  ...
  const handleAddTrip = () => {
    if (newTrip.collectionWeight && newTrip.dischargeWeight && newTrip.collectionTime) {
      const trip = {
        id: Date.now(),
        ...newTrip,
        time: newTrip.collectionTime,
        collectionWeight: parseFloat(newTrip.collectionWeight),
        dischargeWeight: parseFloat(newTrip.dischargeWeight),
      ...
      alert('Por favor, preencha pelo menos o horário da coleta e os pesos.');
    }
  };

  const getDayData = (date) => {
    const dayTrips = trips.filter(trip => trip.date === date);
    const totalCollection = dayTrips.reduce((sum, trip) => sum + trip.collectionWeight, 0);
    const totalDischarge = dayTrips.reduce((sum, trip) => sum + trip.dischargeWeight, 0);
  ...
  const getMonthData = (month) => {
    ...
    monthTrips.forEach(trip => {
      ...
      dailyData[day].collection += trip.collectionWeight;
      dailyData[day].discharge += trip.dischargeWeight;
                        ...
                        <div className="flex-1">
                          <div className="bg-gray-200 rounded-full h-2 sm:h-3">
                            <div 
                              ...
                              style={{ width: `${selectedDayData.trips.length > 0 ? (trip.collectionWeight / Math.max(...selectedDayData.trips.map(t => t.collectionWeight))) * 100 : 0}%` }}
                            ></div>
                          </div>
                        </div>
                        <div className="flex-1">
                          <div className="bg-gray-200 rounded-full h-2 sm:h-3">
                            <div 
                              ...
                              style={{ width: `${selectedDayData.trips.length > 0 ? (trip.dischargeWeight / Math.max(...selectedDayData.trips.map(t => t.dischargeWeight))) * 100 : 0}%` }}
                            ></div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="bg-gray-200 rounded-full h-2">
                          <div 
                            ...
                            style={{ width: `${Object.keys(monthData).length > 0 ? (data.collection / Math.max(...Object.values(monthData).map(d => d.collection))) * 100 : 0}%` }}
                          ></div>
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="bg-gray-200 rounded-full h-2">
                          <div 
                            ...
                            style={{ width: `${Object.keys(monthData).length > 0 ? (data.discharge / Math.max(...Object.values(monthData).map(d => d.discharge))) * 100 : 0}%` }}
                          ></div>
                        </div>
                      </div>
                    </div>
